...
- **THEN** 系统下载图片到本地指定目录，并在数据库的 `catalogs` 表中记录推文 ID、内容、所有下载的本地图片相对路径（以逗号 `,` 分隔拼接成的字符串，存入 `image_path` 字段），初始状态设为 `pending`
...
- **WHEN** 用户通过命令行参数 `--tweet-url` 指定导入某一条特定的推文链接
- **THEN** system 直接抓取并进行图片下载与落库，自动忽略并绕过 `tweet_analysis` 的文本 LLM 过滤


### Requirement: 推文内容与图片关联管理
系统 MUST 将下载的品书图片与社团 ID、推文 ID 建立明确的外键关联，以方便后续制品提取与查询。一条推文即使包含多张图片，也 MUST 只在数据库 `catalogs` 表中占用一行记录。

#### Scenario: 品书图片下载并与社团关联成功
- **WHEN** 推文图片成功下载到本地
- **THEN** 系统在 `catalogs` 表中写入对应的 `circle_id`、`tweet_id`（原始推文 ID，不带 `_idx` 后缀）和本地文件路径 `image_path`（如果有多张图片，则用逗号 `,` 分隔将多个路径拼接为单个字符串存入）
