...
- **Comiket 倾向**：漫画与插画集（同人志）占比超过 90%，文本小说极度边缘化。通过对 C108 描述文本（`description` 字段）的检索，小说及轻小说（小説、ライトノベル）的提及率仅为 **5.5%** 左右。这与 Pixiv 的视觉驱动社交属性以及日本对“画作”的实体消费历史契合`*(注：此小说提及率 5.5% 来源于对 C108 社团描述 of 的实证分析；而日本对“画作”的实体消费倾向主要基于行业共识与受众行为的定性观察)*`。
