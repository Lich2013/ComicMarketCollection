...
为了量化题材大盘的头部集聚效应，本研究引入行业集中度指标 $CR_n$：

$$CR_n = \sum_{i=1}^{n} P_i$$

其中 $P_i$ 为 Comiket 中按摊位数占比排名前 $i$ 题材的占比。
...
在产业经济学中，贝恩（Bain）根据 $CR_4$ 或 $CR_8$ 划分市场集中度。若 $CR_8 \ge 70\%$ 或 $CR_{10} \ge 60\%$，说明该市场具有**“中度寡占型 (Moderately Oligopolistic)”**特征。本数据集的 $CR_{10}$ 达到了 $61.38\%$，说明 Comiket 的摊位分配呈现中度聚集状态，绝大多数物理空间和流量被头部的 10 个主力题材牢牢掌控，但同时长尾端依然留有近 $39\%$ 的空间承载其他 28 个垂直题材，表现出“寡占与长尾并存”的双层亚文化生态。

> **【通俗直观解释】**：这里使用的 $CR_n$ 代表前 $n$ 个最大题材占整个同人展位数量的累计百分比。计算发现，仅排名前 10 的大题材就霸占了整个 Comiket 61% 的摊位。按照产业经济学的标准，这说明 Comiket 展位分布属于“中度集中”状态——少数大热门题材分走了超过六成的流量和空间，不过剩下的近四成空间依然分给了 28 个小众题材，呈现出“大头聚集、小尾丰富”的生态。
...
同人偏离度指数 DBI 在数学上与经济地理学及区域经济学中常用的经典指标**区位商 (Location Quotient, 简写为 LQ)** 完全同构。
在地理学中，区位商衡量一个区域在特定行业上的专业化集聚程度：
$$LQ_i = \frac{e_i / e}{E_i / E}$$

> **【通俗直观解释】**：同人偏离度 (DBI) 的实质就是地理学中的“区位商 (LQ)”。它用“这个题材在 Comiket 摊位里的占比”除以“它在大众二次元圈子里的占比”。
> *   如果算出来 **DBI 远大于 1**（比如《碧蓝档案》是 1.91），说明这题材在同人会场里“超量聚集”，创作者非常愿意为其出本；
> *   如果 **DBI 远小于 1**（比如《赛马娘》是 0.58），说明虽然大众很火，但在同人展里却意外冷清（可能受版权规制或政策影响）。
...
我们将题材的 Comiket 比例与大盘受众热度之比定义为 **DBI (Doujin Bias Index)**。其计算公式如下：
...
在 DBI 的计算中，“该题材在大众二次元大盘市场中的受众热度占比”是重要的基准分母。为确保研究的科学性并克服离线计算的实时网络限制，本数据采用**多源数据校准的静态基准线**方案，预设于代码 [src/analytics.py](file:///Users/lich/work/comicMarketCollection/src/analytics.py#L6-L23) 的 `AUDIENCE_POPULARITY_BASELINE` 中。

关于此基线数据在设计阶段的手工估算口径、平台指标详情以及如何自定义更新，请参阅专门的说明文件：**[Comic Market 大众受众热度基线估算说明书](file:///Users/lich/work/comicMarketCollection/research/audience_baseline_methodology.md)**。
...
## 附录：核心统计 SQL
*   数据库查询脚本源码详见：[research/sql/genre_distribution_dbi.sql](file:///Users/lich/work/comicMarketCollection/research/sql/genre_distribution_dbi.sql) (可用 SQLite 客户端直接执行，用于统计全局题材分布及计算 DBI 指数)。
